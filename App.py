...
# --- NUEVA FUNCIÓN: INTERACCIÓN (¿ESTÁS VIVO?) ---
        ...
        if response["result"]:
            ...
            if msg_id > st.session_state.ultimo_id_leido:
                ...
                if "/status" in low_msg or "vivo" in low_msg:
                    enviar_telegram("🔥 *¡Activo y pendiente, patrón!* No estaba dormido, estaba acechando las murallas. ¿Qué se cuenta?")
...
st.set_page_config(page_title="Raúl Sniper V8.1 Full", layout="wide")
...
def procesar_sniper_v8(s):
    try:
        ...
        if v_act['High'] == v_act['Low']: return # Mercado cerrado o muerto
        ...
        if dist <= 0.018 and max_t >= 35:
            ...
            elif precio > best_p and (v_act['Open'] - v_act['Low']) > (tamanio *
